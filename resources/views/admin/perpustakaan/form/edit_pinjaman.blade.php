        ...
        <div class="card border-top border-0 border-4 border-white">
            <div class="card-body">
                <div class="border p-4 rounded">
                    ...
                    <hr/>
                    <form action="/pinjaman/edit" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row mb-3">
                            <label for="inputChoosePassword2" class="col-sm-3 col-form-label">Mahasiswa</label>
                            <div class="col-sm-9">
                                <input type="hidden" name="id" value="{{ $pinjaman->id_pinjaman }}">
                                <select name="id_mahasiswa" class="single-select">
                                    <option value="">-- Choose --</option>
                                    @foreach ($mahasiswa as $m)
                                        <option value="{{ $m->id }}" {{ $m->id == $pinjaman->id_mahasiswa ? 'selected' : '' }}>
                                            {{ $m->id_kelas }} | {{ $m->nama_mahasiswa }} - {{ $m->nama_kelas }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <select name="id_buku" class="single-select">
                                    <option value="">-- Choose --</option>
                                    @foreach ($buku as $b)
                                        <option value="{{ $b->id_buku }}" {{ $b->id_buku == $pinjaman->id_buku ? 'selected' : '' }}>
                                            {{ $b->jenis_buku }} | {{ $b->judul_buku }} - {{ $b->tahun_terbit }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <input type="date" name="tgl_pinjam" value="{{ $pinjaman->tgl_pinjam }}" readonly class="form-control">
                            </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                @php
                                    $tgl_pinjam = new DateTime($pinjaman->tgl_pinjam);
                                    $tgl_jt = new DateTime($pinjaman->tgl_jatuh_tempo);
                                    $lama_pinjam = $tgl_jt->diff($tgl_pinjam);
                                @endphp
                                <input type="number" name="lama_pinjam" value="{{ $lama_pinjam->d }}" class="form-control">
                            </div>
                        </div>
                        <div class="row">
